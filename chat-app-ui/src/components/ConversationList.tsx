              ...
              <button
                onClick={(e) => {
                  ...
                  if (confirm('¿Eliminar grupo?')) {
                    import('../stores/groupsStore').then(({ useGroupsStore }) => {
                      useGroupsStore.getState().eliminarGrupo(grupo.id)
                        .catch(err => alert('No se pudo eliminar el grupo. Solo el creador puede eliminarlo.'));
                    });
                  }
                }}
